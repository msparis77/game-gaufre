  ...
  return(
    <div style={{background:S.bg,minHeight:"100vh",color:S.text,fontFamily:"system-ui,sans-serif",maxWidth:560,margin:"0 auto",paddingBottom:60}} onClick={touch}>
      ...
      {tab==="gaming"&&<div style={{padding:14}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
          {stations.map(st=>{const ses=sessions[st.id];const active=!!ses;return(
            <div key={st.id} style={{background:active?"#051505":S.card,border:`2px solid ${active?S.green:S.border}`,borderRadius:12,padding:12}}>
              ...
              {user.role==="patron"&&<div style={{display:"flex",gap:4,marginTop:6}}>
        ...
        {user.role==="patron"&&<button onClick={()=>setAddStationModal(true)} style={{width:"100%",background:"transparent",border:`2px dashed ${S.gold}`,borderRadius:10,padding:"9px",cursor:"pointer",color:S.gold,fontWeight:700,fontSize:13,marginBottom:12}}>＋ Ajouter une console / un poste</button>}
      ...
      {tab==="planning"&&<div style={{padding:14}}>
        ...
        {user.role==="patron"&&<button onClick={()=>setEditRec({id:"",emoji:"🍽️",name:"",category:"Crêpes",snackId:"",ingredients:[]})} style={{width:"100%",background:"transparent",border:`2px dashed ${S.orange}`,borderRadius:10,padding:"9px",cursor:"pointer",color:S.orange,fontWeight:700,fontSize:13,marginBottom:12}}>＋ Ajouter une recette</button>}
          ...
          <div key={cat} style={Card()}>
            ...
            {recipes.filter(r=>r.category===cat).map(rec=>(
              <div key={rec.id} style={{marginBottom:10,paddingBottom:10,borderBottom:`1px solid ${S.border}`}}>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:user.role==="patron"?6:0}}>
