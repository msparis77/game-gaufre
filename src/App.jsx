...
const INIT_EXP_CATS=[{id:"c1",emoji:"⚡",label:"Électricité"},{id:"c2",emoji:"🏠",label:"Loyer"},{id:"c3",emoji:"👥",label:"Personnel"},{id:"c4",emoji:"📄",label:"Facture"},{id:"c5",emoji:"🔧",label:"Travaux"}];
...
const fbSave=async(key,data)=>{try{await fetch("/api/store",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({key,data})});}catch(e){}};const fbGet=async(key)=>{try{const r=await fetch("/api/store?key="+encodeURIComponent(key));if(!r.ok)return null;return await r.json();}catch(e){return null;}};function exportClosingPDF(d){if(!window.jspdf){alert("PDF non disponible");return;}const{jsPDF}=window.jspdf;const doc=new jsPDF();let y=20;doc.setFontSize(16);doc.text(`Cloture de caisse - ${d.storeName}`,14,y);y+=8;doc.setFontSize(10);doc.text(`${d.dateStr} - ${d.employeeName}`,14,y);y+=10;doc.setFontSize(12);const ln=(l,v)=>{doc.text(String(l),14,y);doc.text(String(v),120,y);y+=7;};ln("CA total",fmt(d.totalCA));ln("  dont Food",fmt(d.totalFood));ln("  dont Gaming",fmt(d.totalGaming));ln("Depenses",fmt(d.totalExpenses));ln("Benefice net",fmt(d.netProfit));ln("Tickets emis",d.ticketCount);y+=3;ln("Caisse comptee",fmt(d.cashTotal));ln("Ecart",fmt(d.cashDiff));y+=6;if(d.top5&&d.top5.length){doc.setFontSize(12);doc.text("Top produits",14,y);y+=7;doc.setFontSize(10);d.top5.forEach(p=>{doc.text(`${p.emoji} ${p.name} x${p.sold}`,16,y);y+=6;});y+=3;}if(d.lossAlerts&&d.lossAlerts.length){doc.setFontSize(12);doc.setTextColor(200,0,0);doc.text("Alertes perte",14,y);y+=7;doc.setFontSize(10);d.lossAlerts.forEach(p=>{doc.text(`${p.emoji} ${p.name}: ${p.missing} manquant(s) = ${fmt(p.value)}`,16,y);y+=6;});doc.setTextColor(0,0,0);y+=3;}doc.setFontSize(9);doc.text(`Genere le ${new Date().toLocaleString("fr-FR")}`,14,285);doc.save(`Cloture_${d.storeName.replace(/[^a-zA-Z0-9]/g,"_")}_${d.dateStr.replace(/\//g,"-")}.pdf`);}
function exportClosingExcel(d){if(!window.XLSX){alert("Excel non disponible");return;}const wb=XLSX.utils.book_new();const resume=[["Cloture de caisse",d.storeName],["Date",d.dateStr],["Employe",d.employeeName],[],["CA total",d.totalCA],["Food",d.totalFood],["Gaming",d.totalGaming],["Depenses",d.totalExpenses],["Benefice net",d.netProfit],["Tickets",d.ticketCount],["Caisse comptee",d.cashTotal],["Ecart",d.cashDiff]];XLSX.utils.book_append_sheet(wb,XLSX.utils.aoa_to_sheet(resume),"Resume");const ventes=[["Heure","Produits","Total"]].concat((d.sales||[]).map(s=>[s.time||"",(s.items||[]).map(i=>i.name).join(", "),s.total]));XLSX.utils.book_append_sheet(wb,XLSX.utils.aoa_to_sheet(ventes),"Ventes");const dep=[["Heure","Description","Montant"]].concat((d.expenses||[]).map(e=>[e.time||"",e.label,e.amount]));XLSX.utils.book_append_sheet(wb,XLSX.utils.aoa_to_sheet(dep),"Depenses");if(d.lossAlerts&&d.lossAlerts.length){const pertes=[["Produit","Manquant","Valeur"]].concat(d.lossAlerts.map(p=>[p.name,p.missing,p.value]));XLSX.utils.book_append_sheet(wb,XLSX.utils.aoa_to_sheet(pertes),"Pertes");}XLSX.writeFile(wb,`Cloture_${d.storeName.replace(/[^a-zA-Z0-9]/g,"_")}_${d.dateStr.replace(/\//g,"-")}.xlsx`);}
...
export default function App(){
  ...
  const [purchQtyVal,setPurchQtyVal]=useState("");
  const [purchCostVal,setPurchCostVal]=useState("");
  ...
  const [expCats,setExpCats]=useState(INIT_EXP_CATS);
  ...
  const requirePatron=onSuccess=>{if(user?.role==="patron"||user?.role==="gerant"){onSuccess();return;}setPinModal({onSuccess});};
  const tryPatronModal=code=>{const p=empRef.current.find(e=>(e.role==="patron"||e.role==="gerant")&&code===e.pin);if(p){setPinModal(null);setPinMErr(false);pinModal.onSuccess();}else{setPinMErr(true);setTimeout(()=>setPinMErr(false),1200);}};
  ...
  const ingPurchased=id=>purchases.filter(p=>p.ingId===id).reduce((s,p)=>s+p.qty,0);
  ...
  const productCost=p=>p.dIngId?(p.dIngQty||0)*(ingredients.find(i=>i.id===p.dIngId)?.unitCost||0):recipeCost(recipes.find(r=>r.snackId===p.id));
  const totalPurchasesCost=purchases.reduce((s,p)=>s+p.cost,0);
  const cogsConsumed=ingredients.reduce((s,ing)=>s+ingUsed(ing.id)*ing.unitCost,0);
  const grossMargin=totalCA-cogsConsumed;
  const realNetProfit=grossMargin-totalExpenses;
  ...
  const scanIA=async(file,target)=>{
    ...
    try{
      ...
      if(parsed.stocks){
        ...
        if(nbNew>0){setIngredients(newIngList);saveProds(boissons,snacks,newIngList,recipes,stations,photoPrice,dailyGoal,ticketNo);}
  ...
  const recordPurchase=()=>{if(!purchModal||!purchQtyVal||!purchCostVal)return;const qty=parseFloat(purchQtyVal)||0;const cost=parseFloat(purchCostVal)||0;if(qty<=0||cost<=0)return;const oldQty=ingRem(purchModal.id);const oldCost=purchModal.unitCost||0;const newQty=oldQty+qty;const newUnitCost=newQty>0?Math.round(((oldQty*oldCost)+cost)/newQty):oldCost;const ni=ingredients.map(x=>x.id===purchModal.id?{...x,unitCost:newUnitCost}:x);setIngredients(ni);const pu={id:uid(),ingId:purchModal.id,ingName:purchModal.name,ingEmoji:purchModal.emoji,unit:purchModal.unit,qty,cost,prevUnitCost:oldCost,note:purchNote.trim(),by:user?.name,time:timeStr()};const np=[...purchases,pu];setPurchases(np);saveProds(boissons,snacks,ni,recipes,stations,photoPrice,dailyGoal,ticketNo);saveDay({purchases:np});addAudit("ACHAT",`${purchModal.emoji}${purchModal.name} +${fmtQ(qty,purchModal.unit)} pour ${fmt(cost)}`);setPurchModal(null);setPurchQtyVal("");setPurchCostVal("");setPurchNote("");showToast(`✓ Achat: ${fmtQ(qty,purchModal.unit)} ${purchModal.name}`);};
  const addExpCat=()=>{const emoji=window.prompt("Emoji de la catégorie ?","🧾")||"🧾";const label=window.prompt("Nom de la catégorie ?");if(!label)return;const nc=[...expCats,{id:uid(),emoji,label}];setExpCats(nc);saveProdsMerge({ec:nc});};
  const editExpCat=cat=>{const emoji=window.prompt("Emoji ?",cat.emoji)||cat.emoji;const label=window.prompt("Nom ?",cat.label);if(!label)return;const nc=expCats.map(c=>c.id===cat.id?{...c,emoji,label}:c);setExpCats(nc);saveProdsMerge({ec:nc});};
  const deleteExpCat=id=>{if(!window.confirm("Supprimer cette catégorie ?"))return;const nc=expCats.filter(c=>c.id!==id);setExpCats(nc);saveProdsMerge({ec:nc});};
...
COÛT MATIÈRES CONSOMMÉES: ${fmt(cogsConsumed)} | MARGE RÉELLE: ${fmt(grossMargin)}`;
  ...
  if(!user)return(
   ...
   {!storeConfirmed?<div style={{width:"100%",maxWidth:340,marginTop:10}}><div style={{color:S.muted,fontSize:13,textAlign:"center",marginBottom:16}}>Choisissez votre boutique pour continuer</div>{STORES.map(store=><button key={store.id} onClick={()=>{setCurrentStore(store);try{localStorage.setItem("gg3-last-store",store.id);}catch(e){}setStoreConfirmed(true);}} style={{display:"flex",alignItems:"center",gap:12,width:"100%",background:store.id===currentStore.id?S.card2:S.card,border:`2px solid ${store.id===currentStore.id?S.gold:S.border}`,borderRadius:10,padding:"12px 14px",cursor:"pointer",color:S.text,marginBottom:10,textAlign:"left"}}><span style={{fontSize:28}}>{store.emoji}</span><div style={{flex:1}}><div style={{fontSize:13,fontWeight:700,color:store.id===currentStore.id?S.gold:S.text}}>{store.name}</div><div style={{fontSize:10,color:S.muted,marginTop:2}}>{store.id===currentStore.id?"Dernière boutique utilisée":"Appuyez pour sélectionner"}</div></div>{store.id===currentStore.id&&<span style={{color:S.gold,fontSize:18}}>✓</span>}</button>)}</div>:<>
     ...
     <div style={{marginTop:20,display:"flex",flexWrap:"wrap",justifyContent:"center",gap:6}}>{emps.map(e=><div key={e.id} style={{background:S.card,border:`1px solid ${e.role==="patron"?S.gold:e.role==="gerant"?S.blue:S.border}`,borderRadius:8,padding:"4px 10px",fontSize:11,color:e.role==="patron"?S.gold:e.role==="gerant"?S.blue:S.muted}}>{e.role==="patron"?"👑":e.role==="gerant"?"⭐":"👤"} {e.name}</div>)}</div>
     </>}
    </div>
  );

  // ══════════════════ MAIN APP ══════════════════
  const isPatron=user.role==="patron",isGerant=user.role==="gerant",isManager=isPatron||isGerant;
  return(
    <div style={{background:S.bg,minHeight:"100vh",color:S.text,fontFamily:"system-ui,sans-serif",maxWidth:560,margin:"0 auto",paddingBottom:60}} onClick={touch}>
      ...
      <div style={{background:S.card,padding:"11px 16px",borderBottom:`3px solid ${S.gold}`,display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,zIndex:100}}>
        <div>
          ...
          <div style={{fontSize:10,color:S.muted,marginTop:1}}>{user.role==="patron"?"👑":user.role==="gerant"?"⭐":"👤"} {user.name}{Object.keys(sessions).length>0&&<span style={{color:S.green,marginLeft:6}}>🎮{Object.keys(sessions).length}</span>}{(lossAlerts.length>0||ingAlerts.length>0)&&<span style={{color:S.red,marginLeft:6}}>⚠️</span>}</div>
        </div>
      ...
      {tab==="home"&&<div style={{padding:14}}>
        ...
        {isManager&&<>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
          {[{l:"COÛT MATIÈRES",v:fmt(cogsConsumed),c:S.orange,sub:`Achats: ${fmt(totalPurchasesCost)}`},{l:"MARGE RÉELLE",v:fmt(grossMargin),c:grossMargin>=0?S.green:S.red,sub:`Net réel: ${fmt(realNetProfit)}`}].map(c=>(<div key={c.l} style={Card()}><div style={{fontSize:9,color:S.muted,letterSpacing:1,marginBottom:4}}>{c.l}</div><div style={{fontSize:16,fontWeight:800,color:c.c}}>{c.v}</div><div style={{fontSize:10,color:S.muted,marginTop:2}}>{c.sub}</div></div>))}
        </div>
        <div style={{...Card(realNetProfit>=0?S.green:S.red)}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:12,fontWeight:700,color:S.muted}}>💰 BÉNÉFICE RÉEL (tout compris)</span><span style={{fontSize:20,fontWeight:800,color:realNetProfit>=0?S.green:S.red}}>{fmt(realNetProfit)}</span></div>
          <div style={{fontSize:10,color:S.muted,marginTop:4}}>CA {fmt(totalCA)} − Matières {fmt(cogsConsumed)} − Charges {fmt(totalExpenses)}</div>
        </div>
        </>}
        <div style={Card()}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontSize:12,fontWeight:700}}>🎯 Objectif {fmt(dailyGoal)}</span>{isManager&&<button onClick={()=>{setNewGoal(String(dailyGoal));setGoalModal(true);}} style={{background:"transparent",border:`1px solid ${S.border}`,color:S.muted,borderRadius:6,padding:"3px 8px",cursor:"pointer",fontSize:11}}>Modifier</button>}</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:12}}>
          {(cTab==="boissons"?boissons:snacks).map(p=>(
            <div key={p.id}>
              ...
              {isManager&&<div style={{display:"flex",gap:2,marginTop:2}}>
                ...
                <button onClick={()=>{if(!window.confirm("Supprimer "+p.name+"?"))return;if(cTab==="boissons"){const nb=boissons.filter(x=>x.id!==p.id);setBoissons(nb);saveProds(nb,snacks,ingredients,recipes,stations,photoPrice,dailyGoal,ticketNo);}else{const ns=snacks.filter(x=>x.id!==p.id);setSnacks(ns);saveProds(boissons,ns,ingredients,recipes,stations,photoPrice,dailyGoal,ticketNo);}addAudit("SUPPR",p.name);showToast("Supprime",S.red);}} style={{flex:1,background:S.card3,border:`1px solid ${S.red}`,color:S.red,borderRadius:5,padding:"2px 0",cursor:"pointer",fontSize:10}}>🗑</button>
              </div>}
            </div>
          ))}
        </div>
        ...
        {stSub==="ing"&&<div>
          ...
          {ingredients.map(ing=>{const s=ingStock[ing.id]||{};const used=ingUsed(ing.id);const rem=ingRem(ing.id);const al=ingAlert(ing.id);return(<div key={ing.id} style={{background:al?"#1a0000":S.card,border:`1px solid ${al?S.red:S.border}`,borderRadius:10,padding:12,marginBottom:8}}>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"flex-end"}}>
              ...
              <button onClick={()=>{setPurchModal(ing);setPurchQtyVal("");setPurchCostVal("");setPurchNote("");}} style={{background:S.card2,border:`1px solid ${S.green}`,color:S.green,borderRadius:6,padding:"8px 10px",cursor:"pointer",fontSize:16}}>📥</button><button onClick={()=>{setExitModal(ing);setExitQtyVal("");setExitNote("");}} style={{background:S.card2,border:`1px solid ${S.orange}`,color:S.orange,borderRadius:6,padding:"8px 10px",cursor:"pointer",fontSize:16}}>📤</button>
            </div>
          ...
          <button onClick={()=>setAddIngModal(true)} style={{width:"100%",background:"transparent",border:`2px dashed ${S.blue}`,borderRadius:10,padding:"14px",cursor:"pointer",color:S.blue,fontWeight:700,fontSize:13,marginTop:4}}>＋ Ajouter ingrédient</button>
          {manualExits.length>0&&<div style={Card()}><div style={{fontWeight:700,color:S.gold,marginBottom:8,fontSize:11,letterSpacing:1}}>SORTIES MANUELLES DU JOUR</div>{[...manualExits].reverse().map(m=><div key={m.id} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:`1px solid ${S.border}`,fontSize:11}}><div><span style={{color:S.muted}}>{m.time} </span>{m.ingEmoji}{m.ingName}{m.note?` (${m.note})`:""}</div><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{color:S.red,fontWeight:700}}>-{fmtQ(m.qty,m.unit)}</span><button onClick={()=>{const ne=manualExits.filter(x=>x.id!==m.id);setManualExits(ne);saveDay({manualExits:ne});showToast("✓ Supprimé",S.red);}} style={{background:"transparent",border:`1px solid ${S.red}`,color:S.red,borderRadius:4,padding:"1px 5px",cursor:"pointer",fontSize:10}}>🗑</button></div></div>)}</div>}{purchases.length>0&&<div style={Card()}><div style={{fontWeight:700,color:S.gold,marginBottom:8,fontSize:11,letterSpacing:1}}>ACHATS DU JOUR</div>{[...purchases].reverse().map(p=><div key={p.id} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:`1px solid ${S.border}`,fontSize:11}}><div><span style={{color:S.muted}}>{p.time} </span>{p.ingEmoji}{p.ingName}{p.note?` (${p.note})`:""}</div><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{color:S.green,fontWeight:700}}>+{fmtQ(p.qty,p.unit)}</span><span style={{color:S.muted}}>{fmt(p.cost)}</span><button onClick={()=>{const np=purchases.filter(x=>x.id!==p.id);setPurchases(np);saveDay({purchases:np});const ni=ingredients.map(x=>x.id===p.ingId?{...x,unitCost:p.prevUnitCost}:x);setIngredients(ni);saveProds(boissons,snacks,ni,recipes,stations,photoPrice,dailyGoal,ticketNo);showToast("✓ Supprimé",S.red);}} style={{background:"transparent",border:`1px solid ${S.red}`,color:S.red,borderRadius:4,padding:"1px 5px",cursor:"pointer",fontSize:10}}>🗑</button></div></div>)}<div style={{display:"flex",justifyContent:"space-between",marginTop:8,paddingTop:8,borderTop:`1px solid ${S.border}`,fontWeight:700}}><span>Total</span><span style={{color:S.green}}>{fmt(totalPurchasesCost)}</span></div></div>}
        </div>}
        {stSub==="prod"&&<div>
        ...
        {stSub==="finis"&&<div>
          ...
          {snacks.map(p=>{const prod=prodQtyFn(p.id);const sold=soldQty(p.id);const rem=remQty(p.id);const loss=lossQty(p.id);return(<div key={p.id} style={{background:loss!==null&&loss>0?"#1a0000":S.card,border:`1px solid ${loss!==null&&loss>0?S.red:S.border}`,borderRadius:10,padding:12,marginBottom:8}}>
            ...
            {loss!==null&&loss>0&&<div style={{marginTop:6,fontSize:12,color:S.red,fontWeight:700}}>⚠️ {loss} manquant(s) — {fmt(loss*p.price)}</div>}
                  ...
                  {isManager&&<button onClick={()=>setAddProdCat("snacks")} style={{width:"100%",background:"transparent",border:`2px dashed ${S.purple}`,borderRadius:10,padding:"8px",cursor:"pointer",color:S.purple,fontWeight:700,fontSize:12,marginTop:4}}>＋ Ajouter un produit fini</button>}
        </div>}
        {stSub==="verif"&&<div>
      ...
      {tab==="planning"&&<div style={{padding:14}}>
        ...
        <button onClick={()=>setEditRec({id:"",emoji:"🍽️",name:"",category:"Crêpes",snackId:"",ingredients:[]})} style={{width:"100%",background:"transparent",border:`2px dashed ${S.orange}`,borderRadius:10,padding:"9px",cursor:"pointer",color:S.orange,fontWeight:700,fontSize:13,marginBottom:12}}>＋ Ajouter une recette</button>
        {[...new Set(recipes.map(r=>r.category))].map(cat=>(
          <div key={cat} style={Card()}>
            ...
            {recipes.filter(r=>r.category===cat).map(rec=>(
              <div key={rec.id} style={{marginBottom:10,paddingBottom:10,borderBottom:`1px solid ${S.border}`}}>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:isManager?6:0}}>
                ...
                {isManager&&<div style={{display:"flex",gap:4}}>
                  <button onClick={()=>setEditRec({...rec})} style={{flex:1,background:S.card3,border:`1px solid ${S.blue}`,color:S.blue,borderRadius:6,padding:"4px 0",cursor:"pointer",fontSize:10}}>✏️ Modifier</button>
                  <button onClick={()=>{if(!window.confirm("Supprimer "+rec.name+"?"))return;const nr=recipes.filter(x=>x.id!==rec.id);setRecipes(nr);saveProds(boissons,snacks,ingredients,nr,stations,photoPrice,dailyGoal,ticketNo);addAudit("SUPPR REC",rec.name);showToast("✓ Supprimé",S.red);}} style={{flex:1,background:S.card3,border:`1px solid ${S.red}`,color:S.red,borderRadius:6,padding:"4px 0",cursor:"pointer",fontSize:10}}>🗑 Supprimer</button>
                </div>}
              </div>
            ))}
          </div>
      ...
      {tab==="recettes"&&<div style={{padding:14}}>
        <button onClick={()=>setEditRec({id:"",emoji:"🍽️",name:"",category:"Crêpes",snackId:"",ingredients:[]})} style={{width:"100%",background:"transparent",border:`2px dashed ${S.orange}`,borderRadius:10,padding:"10px",cursor:"pointer",color:S.orange,fontWeight:700,fontSize:13,marginBottom:12}}>＋ Nouvelle recette</button>
        {[...new Set(recipes.map(r=>r.category))].map(cat=>(
          <div key={cat} style={Card()}>
            <div style={{fontSize:11,fontWeight:700,color:S.orange,letterSpacing:2,marginBottom:12}}>{cat.toUpperCase()}</div>
            {recipes.filter(r=>r.category===cat).map(rec=>(
      ...
      {tab==="bilan"&&<div style={{padding:14}}>
        ...
        <div style={Card()}><div style={{fontWeight:700,color:S.gold,marginBottom:8,fontSize:11,letterSpacing:1}}>VENTES RÉCENTES</div>{sales.length===0?<div style={{color:S.muted,fontSize:13,textAlign:"center",padding:12}}>Aucune vente</div>:[...sales].reverse().slice(0,12).map(sale=><div key={sale.id} style={{borderBottom:`1px solid ${S.border}`,paddingBottom:6,marginBottom:6}}><div style={{display:"flex",justifyContent:"space-between",fontSize:12}}><span style={{color:S.muted}}>{sale.time} <span style={{color:S.teal,fontSize:10}}>#{sale.ticketNo||"—"}</span> <span style={{color:S.blue,fontSize:10}}>({sale.by})</span></span><div style={{display:"flex",gap:8,alignItems:"center"}}><span style={{fontWeight:800,color:S.green}}>{fmt(sale.total)}</span>{isManager&&<button onClick={()=>deleteSale(sale.id)} style={{background:"transparent",border:"none",color:S.red,cursor:"pointer",fontSize:13}}>🗑</button>}</div></div><div style={{fontSize:10,color:"#aaa",marginTop:2}}>{sale.items.map(i=>`${i.emoji||""}${i.name}×${i.qty}`).join(" • ")}</div></div>)}</div>
      </div>}
      {/* ══ VUE GLOBALE (PATRON) ══ */}
      {tab==="global"&&isPatron&&<div style={{padding:14}}>
      ...
      {purchModal&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:16}}>
        <div style={{background:S.card,borderRadius:16,padding:24,width:"100%",maxWidth:340,border:`2px solid ${S.green}`}}>
          <div style={{fontWeight:800,fontSize:15,color:S.green,marginBottom:4}}>📥 Achat ingrédient</div>
          <div style={{fontSize:13,color:S.muted,marginBottom:12}}>{purchModal.emoji} {purchModal.name} (dispo: {fmtQ(ingRem(purchModal.id),purchModal.unit)}, coût actuel: {fmt(purchModal.unitCost)}/{purchModal.unit})</div>
          <input type="number" step="0.01" value={purchQtyVal} placeholder={`Quantité (${purchModal.unit})`} autoFocus onChange={e=>setPurchQtyVal(e.target.value)} style={Inp()}/>
          <input type="number" value={purchCostVal} placeholder="Prix total payé (FCFA)" onChange={e=>setPurchCostVal(e.target.value)} style={{...Inp(),marginTop:8}}/>
          <input type="text" value={purchNote} placeholder="Note (optionnel)" onChange={e=>setPurchNote(e.target.value)} style={{...Inp(),marginTop:8}}/>
          <div style={{display:"flex",gap:10,marginTop:16}}><button onClick={()=>setPurchModal(null)} style={{background:S.card2,border:`1px solid ${S.border}`,color:S.muted,borderRadius:8,padding:"10px",cursor:"pointer",fontSize:13,flex:1}}>Annuler</button><button onClick={recordPurchase} style={{...Btn(S.green),flex:1}}>✓ Confirmer</button></div>
        </div>
      </div>}
        ...
        <div style={{background:S.card,borderRadius:16,padding:24,width:"100%",maxWidth:320,border:`2px solid ${S.blue}`}}>
          ...
          <div style={{display:"flex",gap:10,marginTop:8}}>
            ...
            <button onClick={()=>{
              ...
              if(editProd.cat==="boissons"){const nb=boissons.map(x=>x.id===editProd.id?editProd:x);setBoissons(nb);saveProds(nb,snacks,ingredients,recipes,stations,photoPrice,dailyGoal,ticketNo);}
              else{const ns=snacks.map(x=>x.id===editProd.id?editProd:x);setSnacks(ns);saveProds(boissons,ns,ingredients,recipes,stations,photoPrice,dailyGoal,ticketNo);}
        ...
        <div style={{background:S.card,borderRadius:16,padding:24,width:"100%",maxWidth:320,border:`2px solid ${S.gold}`}}>
          ...
          <div style={{display:"flex",gap:10,marginTop:8}}>
            ...
            <button onClick={()=>{
              ...
              if(addProdCat==="boissons"){const nb=[...boissons,np];setBoissons(nb);saveProds(nb,snacks,ingredients,recipes,stations,photoPrice,dailyGoal,ticketNo);}
              else{const ns=[...snacks,np];setSnacks(ns);saveProds(boissons,ns,ingredients,recipes,stations,photoPrice,dailyGoal,ticketNo);}
        ...
        <div style={{background:S.card,borderRadius:16,padding:20,width:"100%",maxWidth:400,border:`2px solid ${S.orange}`,marginTop:16,marginBottom:20}}>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <button onClick={()=>{
              ...
              setRecipes(nr);
              saveProds(boissons,snacks,ingredients,nr,stations,photoPrice,dailyGoal,ticketNo);
      ...
      {empModal&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.95)",display:"flex",alignItems:"flex-start",justifyContent:"center",zIndex:200,padding:16,overflowY:"auto"}}>
        <div style={{background:S.card,borderRadius:16,padding:20,width:"100%",maxWidth:400,border:`2px solid ${S.gold}`,marginTop:20}}>
          ...
          {editEmp?<div><div style={{fontSize:12,color:S.muted,marginBottom:12}}>Modifier {editEmp.name}</div>{editEmp.role!=="patron"&&<div style={{marginBottom:12}}><div style={{fontSize:11,color:S.muted,marginBottom:4}}>Rôle</div><div style={{display:"flex",gap:8}}>{[["employe","👤 Employé"],["gerant","⭐ Gérant"]].map(([rv,rl])=><button key={rv} onClick={()=>setEditEmp(p=>({...p,role:rv}))} style={{flex:1,background:editEmp.role===rv?S.gold:S.card2,color:editEmp.role===rv?S.bg:S.text,border:`1px solid ${editEmp.role===rv?S.gold:S.border}`,borderRadius:8,padding:"8px",cursor:"pointer",fontSize:12,fontWeight:700}}>{rl}</button>)}</div></div>}{[{l:"Nom",k:"name",t:"text"},{l:"PIN (4 chiffres)",k:"pin",t:"number"}].map(f=><div key={f.k} style={{marginBottom:12}}><div style={{fontSize:11,color:S.muted,marginBottom:4}}>{f.l}</div><input type={f.t} value={editEmp[f.k]} onChange={e=>setEditEmp(p=>({...p,[f.k]:f.k==="pin"?e.target.value.slice(0,4):e.target.value}))} style={Inp()}/></div>)}<div style={{display:"flex",gap:10}}><button onClick={()=>setEditEmp(null)} style={{background:S.card2,border:`1px solid ${S.border}`,color:S.muted,borderRadius:8,padding:"10px",cursor:"pointer",fontSize:13,flex:1}}>Annuler</button><button onClick={async()=>{if(editEmp.pin.length===4&&editEmp.name){const ne=emps.map(e=>e.id===editEmp.id?editEmp:e);setEmps(ne);empRef.current=ne;await saveEmps(ne);setEditEmp(null);showToast("✓ Mis à jour");}}} disabled={editEmp.pin.length!==4} style={{...Btn(S.gold),flex:1,opacity:editEmp.pin.length===4?1:0.4}}>✓ Sauvegarder</button></div></div>
          :<div>{emps.map(e=><div key={e.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:`1px solid ${S.border}`}}><div style={{fontSize:22}}>{e.role==="patron"?"👑":e.role==="gerant"?"⭐":"👤"}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:700,color:e.role==="patron"?S.gold:e.role==="gerant"?S.blue:S.text}}>{e.name}</div><div style={{fontSize:10,color:S.muted}}>PIN: {"•".repeat(4)} • {e.role}</div></div><button onClick={()=>setEditEmp({...e})} style={{background:S.card2,border:`1px solid ${S.blue}`,color:S.blue,borderRadius:8,padding:"6px 10px",cursor:"pointer",fontSize:13}}>✏️</button></div>)}<button onClick={()=>setEmpModal(false)} style={{background:S.card2,border:`1px solid ${S.border}`,color:S.muted,borderRadius:8,padding:"10px",cursor:"pointer",fontSize:13,width:"100%",marginTop:12}}>Fermer</button></div>}
        </div>
      </div>}
      ...
      {expModal&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:16}}>
        <div style={{background:S.card,borderRadius:16,padding:24,width:"100%",maxWidth:340,border:`2px solid ${S.orange}`}}>
          ...
          <div style={{fontSize:11,color:S.muted,marginBottom:12}}>Toutes les charges : électricité, loyer, personnel, factures, travaux...</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:10}}>
            {expCats.map(c=><button key={c.id} onClick={()=>setNewExp(p=>({...p,emoji:c.emoji,label:c.label}))} style={{background:S.card2,border:`1px solid ${S.border}`,color:S.text,borderRadius:20,padding:"6px 10px",cursor:"pointer",fontSize:11}}>{c.emoji} {c.label}</button>)}
            <button onClick={()=>setManageCats(m=>!m)} style={{background:"transparent",border:`1px dashed ${S.muted}`,color:S.muted,borderRadius:20,padding:"6px 10px",cursor:"pointer",fontSize:11}}>⚙️ Gérer</button>
          </div>
          {manageCats&&<div style={{background:S.card2,borderRadius:8,padding:8,marginBottom:12}}>
            {expCats.map(c=><div key={c.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"4px 0",borderBottom:`1px solid ${S.border}`,fontSize:12}}><span>{c.emoji} {c.label}</span><div style={{display:"flex",gap:6}}><button onClick={()=>editExpCat(c)} style={{background:"transparent",border:`1px solid ${S.blue}`,color:S.blue,borderRadius:4,padding:"1px 6px",cursor:"pointer",fontSize:10}}>✏️</button><button onClick={()=>deleteExpCat(c.id)} style={{background:"transparent",border:`1px solid ${S.red}`,color:S.red,borderRadius:4,padding:"1px 6px",cursor:"pointer",fontSize:10}}>🗑</button></div></div>)}
            <button onClick={addExpCat} style={{width:"100%",background:"transparent",border:`1px dashed ${S.teal}`,color:S.teal,borderRadius:6,padding:"6px",cursor:"pointer",fontSize:11,marginTop:6}}>＋ Nouvelle catégorie</button>
          </div>}
          {[{l:"Emoji",k:"emoji",t:"text",ph:"🛒"},{l:"Description",k:"label",t:"text",ph:"Ex: Facture SENELEC Août"},{l:"Montant (FCFA)",k:"amount",t:"number",ph:"5000"}].map(f=><div key={f.k} style={{marginBottom:12}}><div style={{fontSize:11,color:S.muted,marginBottom:4}}>{f.l}</div><input type={f.t} value={newExp[f.k]} placeholder={f.ph} onChange={e=>setNewExp(p=>({...p,[f.k]:e.target.value}))} style={Inp()}/></div>)}
